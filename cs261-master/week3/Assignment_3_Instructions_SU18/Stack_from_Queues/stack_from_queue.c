 ...
 * allocating the memory, initializing all of the values and returning a pointer
...
/*
 * This function adds a new link and value to the back of the queue.
 * It takes a list and a type variable, allocates the memory and
 * adjusts the proper links to make the connection. No return value.
 */
...
void listQueueRemoveFront(struct listQueue *q) {
  ...
  q->firstLink->next = tmp->next;
  free(tmp);
  q->size--;
  if (listQueueIsEmpty(q)) // set lastlink back to sentinel is list is empty
...
/*
 * This function initializes the values of the created Stack.
 * Initializes both queues and the structSize.
 */
void linkedListStackInit(struct linkedListStack *s) {
  // FIXME: you must write this
  struct listQueue *q1 = listQueueCreate();
  struct listQueue *q2 = listQueueCreate();
  assert(q1 != 0 && q2 != 0);
  s->Q1 = q1;
  s->Q2 = q2;
  s->structSize = 0;
 ...
 * The funciton then also increases the size of the stack by 1.
 */
 ...
 * values that are currently on the stack to the other queue.
 * The stack top is maintained at the back of the queue list.
 */
void linkedListStackPop(struct linkedListStack *s) {
  ...
  // swap queues to maintain Q1 as the main queue
 ...
 * is maintaing the values of the stack.
 */
TYPE linkedListStackTop(struct linkedListStack *s) {
  // FIXME: you must write this
  // put all elements in Q1 to Q2 to get to the top of the stack
  while (s->Q1->size > 1) {
    listQueueAddBack(s->Q2, listQueueFront(s->Q1));
    listQueueRemoveFront(s->Q1);
  }

  int top = listQueueFront(s->Q1); // this is the top of the stack
  listQueueAddBack(s->Q2, listQueueFront(s->Q1)); // add the last value to Q2
  listQueueRemoveFront(s->Q1); // empty Q1 so we can swap Q2 to Q1
